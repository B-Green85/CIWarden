...
# ``\$(cat PROMPT.md)`` and ``\"`` are escaped so they reach osascript untouched —
# the Terminal shell (now cd'd into the staging dir) reads PROMPT.md itself.
_LAUNCH_HEADER = r"""#!/usr/bin/env bash
# Auto-generated by the Conductor — launches one Terminal window per agent.
# Each window cd's into the agent's staging dir and runs claude on its PROMPT.md.
...
tell application "Terminal"
    activate
    do script "cd '$1' && claude --dangerously-skip-permissions -p \"\$(cat PROMPT.md)\""
end tell
OSA
}
...
def write_launch_script(session: ConductorSession) -> Path:
    ...
    that agent's staging dir and running claude on the already-written PROMPT.md.
    """
