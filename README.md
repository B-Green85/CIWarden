...
### CDMAE Enforcement Layer for Agentic Development
...
It is the mechanical implementation of **CDMAE** — Constraint Driven Machine Assisted Engineering. The principle is simple: generation is optional. Verification is not.
...
CDMAE_LOCAL_DEV=1              # Local dev bypass — disables auth token requirement
CDMAE_DRIFT_THRESHOLD=0.5      # Memory gate drift sensitivity (default 0.3)
```
...
```bash
cp ~/Projects/ci-wrapper/hooks/pre-commit /path/to/your/repo/.git/hooks/pre-commit
...
| `CDMAE_LOCAL_DEV` | No | `0` | Set to `1` to bypass auth token requirement in local dev |
| `CDMAE_DRIFT_THRESHOLD` | No | `0.3` | Memory gate drift sensitivity — raise to allow more architectural change per commit |
| `CDMAE_HTTPS` | No | `0` | Set to `1` to enable HTTPS — self-signed cert generated on first startup |
| `CDMAE_HTTPS_VERIFY` | No | `1` | Set to `0` to skip SSL verification for self-signed certs in the pre-commit hook |
| `CDMAE_MEMORY_CACHE` | No | `0` | Set to `1` to cache memory gate results — skips LLM extraction if source files unchanged |
...
*CDMAE — Constraint Driven Machine Assisted Engineering*
*Copyright (c) 2026 Brandon Green. All rights reserved.*
